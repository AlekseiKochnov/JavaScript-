...
function spisokNum(n, m, count) { // Функция создает массив из цифр в диапозоне n - m
    let res = [];
    let range = Math.abs(n - m);
    for (let i = 0; i < count; ++i){
        let num = Math.round(Math.random() * range);
        res.push(Math.min(n, m) + num);
    }
    return res;
}


console.log(spisokNum(0, 100, 100)); // Вызов функции для создания массива
console.log(spisokNum(2, 5, 50));
console.log(spisokNum(100, -5, 70));
console.log(spisokNum(-3, -10, 42));
...
function strRevers(str) {    // Функция разворачивает строку
    let resRevers = '';
...
console.log(strRevers('Привет, мир!')); // Вызов функии
console.log(strRevers('1'));
...
если это 1-й взрыв, и «танк уничтожен», если это 2-й взрыв. После 2-го взрыва танк считается уничтоженным
...
goTank([true, true, true, true, true, true, true, true, true, true])
console.log();
goTank([true, false, false, false, false, false, false, false, false, true])
console.log();
goTank([false, false, false, true, false, false, false, false, false, false])
console.log();
goTank([false, false, false, false, false, false, false, false, false, false])
...
Задание
Сгенерировать массив чисел 1–31 включительно (числа месяца). Вывести с помощью console.log для каждого 
из чисел строку ${число} января, ${день недели}. День недели 1 января должен задаваться с помощью переменной, 
...
let res = ['Понедельник', 'Вторник', 'Среда', 'Четверг', 'Пятница', 'Субота', 'Воскресение'];
let d = res.indexOf('Вторник');
for (let i = 1; i < 32; i++){
    if (d === res.length){
        d = 0;
    }
    console.log(`${i} января, ${res[d]}`);
    d += 1
}
